...
from doofer.embeddings import  cosine_similarity, updateNoteEmbeddings, getNoteSimilarity, getTextEmbedding
from typing import List
from django.db.models import QuerySet
from doofer.models import Note  # Add this line to import the "Note" model from the "doofer" module
...
def getSimilarToText(text: str,  notes: list[Note],  count = 10):
  ...
  similarNoteIds: list[NoteSummary] = vecsSimilarRanked([textVector], notes, "", count)
  return similarNoteIds

def get_note_by_id(notes: list[Note], id: str) -> Note | None:
  return next((x for x in notes if x.id == id), None) # type: ignore[attr-defined]
...
def notesSimilarRanked(
  ...
  return vecsSimilarRanked([note.title_embedding, note.content_embedding], notes, originalId, count, threshold)
...
def doTextSearch(searchText: str, maxResults: float, uid: str) -> list[Any]:
  ...
  for note in notes:
    if note.title.toLowerCase().includes(searchTextLower) or note.comment.toLowerCase().includes(searchTextLower):
